...
child.on('exit', () => {
    try {
        ...
        if (!result.ok) {
            console.error('Renderer test failed:', result.error);
            process.exit(1);
        }
        console.log('Renderer test passed:', result.details);
        process.exit(0);
    } catch (err) {
        console.error('Failed to read renderer test result', err);
        process.exit(1);
    } finally {
        try {
