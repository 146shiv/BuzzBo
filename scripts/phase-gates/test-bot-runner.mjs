...
child.on('exit', () => {
    try {
        ...
        if (!result.ok) {
            console.error('Bot runner test failed:', result.error);
            process.exit(1);
        }
        console.log('Bot runner test passed:', result.details);
        process.exit(0);
    } catch (err) {
        console.error('Failed to read bot test result', err);
        process.exit(1);
    } finally {
        try {
