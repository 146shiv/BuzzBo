...
child.on('exit', () => {
    try {
        ...
        if (!result.ok) {
            console.error('PATCH round-trip failed:', result.error);
            process.exit(1);
        }
        console.log('PATCH round-trip passed');
        process.exit(0);
    } catch (err) {
        console.error('Failed to read patch test result', err);
        process.exit(1);
    } finally {
        try {
